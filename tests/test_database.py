...
class TestDataBase(TestCase):
    ...
    @mock.patch("testes_unit_bd.app.database.MySQLdb")
    ...
    def test_conn_and_cursor_exist(self):
        ...
        with mock.patch("testes_unit_bd.app.database.DataBase.cursor", create=True) as mock_cursor:
            with mock.patch("testes_unit_bd.app.database.DataBase.conn", create=True) as mock_conn:
    ...
    @mock.patch("testes_unit_bd.app.database.DataBase.conn", create=True)
    ...
    def test_delete_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        self.assertEqual("Data is not a tuple!", error.exception.args[0])
    ...
    def test_update_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        self.assertEqual("New Data is not a list!", error.exception.args[0])

        with self.assertRaises(TypeError) as error:
            DataBase().update_data("", [], "")

        self.assertEqual("Where Data is not a list!", error.exception.args[0])
